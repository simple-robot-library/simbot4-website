...
上述代码中，**Ⅰ** 以下的部分主要是跟协程、任务、任务状态与挂起相关，我们暂且忽略它，后续可以通过 `Job` 快捷的实现。
**Ⅱ** 以下的部分则与 `Bot` 相关，是需要我们手动实现的。
...
它会根据一个 `Job` 类型来实现前文我们提到的那些 **Ⅰ** 以下的那部分内容，比如 `join` 之类的。

因此接下来，我们只需要实现剩下的那些 **Ⅱ** 部分的内容即可。
...
### 定义 FooBotManager 接口 {id="def-foo-bot-manager"}

### 实现 FooBotManager 接口 {id="impl-foo-bot-manager"}
